...
check = true
    ...
    const designWidth = _CCSettings.designWidth
    const designHeight = _CCSettings.designHeight

    let rotated = false
    // var paused = false;
    const canvas = document.getElementById('GameCanvas')
    const btnRotate = document.getElementById('btn-rotate')
    const optsDevice = document.getElementById('opts-device')
    const btnPause = document.getElementById('btn-pause')
    const btnStep = document.getElementById('btn-step')
    const optsDebugMode = document.getElementById('opts-debug-mode')
    const btnShowFPS = document.getElementById('btn-show-fps')
    const inputSetFPS = document.getElementById('input-set-fps')
...
return (arr[2])
    ...
    function getEmulatedScreenSize () {
        let w; let h
        const idx = optsDevice.value
        if ( idx === '0' ) {
            w = designWidth
            h = designHeight
        } else {
            const info = devices[parseInt(idx) - 1]
            w = info.width
            h = info.height
        }
        return {
    ...
    function showSplash () {
        ...
        const size = isFullScreen() ? document.documentElement.getBoundingClientRect() : getEmulatedScreenSize()
        ...
        if (size.width < size.height) {
            ...
            const logoDisplayH = size.width * LOGO_SIZE / LOGO_IMG_L_W * LOGO_IMG_L_H
    ...
    function initPreviewOptions () {
        const defaultDevice = getCookie('device')
        const defaultRotate = getCookie('rotate')

        const hasDefaultDevice = defaultDevice !== null
        const hasDefaultRotate = defaultRotate !== null
    ...
    function onload () {
        ...
        btnShowFPS.addEventListener('click', function () {
            const show = !cc.debug.isDisplayStats()
